...
test.describe('Pricing Form', () => {
  ...
  test('should display pricing form with all fields', async ({ page }) => {
    ...
    await expect(page.getByLabel(/free practice audit/i)).toBeVisible();
  ...
  test('should submit pricing request successfully', async ({ page }) => {
    ...
    await page.getByLabel(/free practice audit/i).check();

    // Submit form
    await page.getByRole('button', { name: /get pricing/i }).click();

    // Should show success message
    await expect(page.getByText(/thank you.*pricing request.*received/i)).toBeVisible({ timeout: 10000 });
  ...
  test('should display pricing information', async ({ page }) => {
    ...
    await expect(page.getByText(/percentage|flat fee|custom/i)).toBeVisible();
  });
});
